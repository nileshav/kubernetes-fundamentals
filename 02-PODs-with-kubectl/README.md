...
- **Important Note:**
  ...
  - The below will suffice to create a Pod as a pod without creating deployment. We dont need to add **--generator=run-pod/v1** because --generator flag has been removed from kubectl as of Kubernetes v1.18 and completely deprecated in later versions. **--restart=Never** tells kubectl to create a Pod (not a Deployment). This is the modern equivalent of the old **--generator=run-pod/v1**
```
kubectl run my-first-pod --image stacksimplify/kubenginx:1.0.0 --restart=Never
```  
